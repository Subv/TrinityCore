    ...
    if (criteriaResult)
    {
        ...
        do
        {
            ...
            if (!criteria)
            {
                ...
                sLog->outError(LOG_FILTER_SQL, "Non-existing achievement criteria %u data removed from table `guild_achievement_progress`.", id);
...
{
    WorldPacket data(SMSG_GUILD_CRITERIA_DATA, 4 + 8 + 4 + 4 + 8 + 4 + 4 + 4);

    data << uint32(1);
    data << uint64(progress->counter);

    time_t now = time(NULL);
    data << uint32(now - progress->date);
    data << uint32(secsToTimeBitFields(progress->date));
    data << uint64(progress->counter);
    data << uint32(now - progress->date);
    data << uint32(entry->ID);
    ...
    for (AchievementCriteriaEntryList::const_iterator i = achievementCriteriaList.begin(); i != achievementCriteriaList.end(); ++i)
    {
        ...
        switch (type)
        {
            ...
            case ACHIEVEMENT_CRITERIA_TYPE_WIN_RATED_ARENA:
                ...
                if (achievementCriteria->additionalRequirements[0].additionalRequirement_type == ACHIEVEMENT_CRITERIA_CONDITION_NO_LOSE)
                {
                    ...
                    if (!data || !data->Meets(referencePlayer, unit, miscValue1))
                    {
                        ...
                        SetCriteriaProgress(achievementCriteria, 0, referencePlayer);
                        continue;
                    }
                }
            ...
            case ACHIEVEMENT_CRITERIA_TYPE_EQUIP_EPIC_ITEM:
            {
                ...
                AchievementCriteriaDataSet const* data = sAchievementMgr->GetCriteriaDataSet(achievementCriteria);
                if (!data || !data->Meets(referencePlayer, 0, miscValue1))
                    continue;
            ...
            case ACHIEVEMENT_CRITERIA_TYPE_ROLL_GREED_ON_LOOT:
            {
                ...
                AchievementCriteriaDataSet const* data = sAchievementMgr->GetCriteriaDataSet(achievementCriteria);
                if (!data || !data->Meets(referencePlayer, 0, pProto->ItemLevel))
                    continue;
...
{
    sLog->outInfo(LOG_FILTER_ACHIEVEMENTSYS, "AchievementMgr<Guild>::CompletedAchievement(%u)", achievement->ID);
...
void AchievementMgr<T>::SendAllAchievementData(Player* /*receiver*/) const
{
    ...
    uint32 flagBytesCount = uint32(ceil(float(numCriteria) * 2.0f / 8.0f));

    WorldPacket data(SMSG_ALL_ACHIEVEMENT_DATA, 4 + 4 + (numAchievements * (4 + 4)) + (numCriteria * (8 + 4 + 4 + 8 + 4 + 4)) + flagBytesCount);
    data << uint32(numAchievements);
