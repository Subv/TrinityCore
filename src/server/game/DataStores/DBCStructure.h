...
struct AreaTableEntry
{
    ...
    //float maxDepth                                       // 17 MaxDepth? -500 default value, -1400 on submerged areas (Vash'Jir, etc)
                                                           // 18-19 unknown
...
struct CreatureTypeEntry
{
    ...
    //char* Name;                                     // 1       name
                                                      // 2       string flags
    //uint32 no_experience;                           // 3 no exp? critters, non-combat pets, gas cloud.
};
...
struct ItemRandomPropertiesEntry
{
    ...
    uint32 enchant_id[MAX_ITEM_ENCHANTMENT_EFFECTS];     // 2-6      m_Enchantment
                                                         // 7-8      unused
    char* nameSuffix;                                    // 9        m_name
};

struct ItemRandomSuffixEntry
{
    uint32 ID;                                           // 0        m_ID
    char* nameSuffix;                                    // 1        m_name
                                                         // 2        m_internalName
...
struct MapEntry
{
    ...
    //uint32 parentMap;                                    // 19 mapid of the parent map, phasing
...
struct SkillLineEntry
{
    ...
    char* name;                                          // 3        m_displayName
    //char* description;                                 // 4        m_description
    uint32 spellIcon;                                    // 5        m_spellIconID
    //char* alternateVerb;                               // 6        m_alternateVerb
