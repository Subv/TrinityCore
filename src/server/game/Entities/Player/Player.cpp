...
bool Player::BuildEnumData(PreparedQueryResult result, WorldPacket* data)
{
    ...
    *data << fields[12].GetFloat();                         // z
    ...
    if (uint32 guildId = fields[13].GetUInt32())
        *data << uint64(MAKE_NEW_GUID(fields[13].GetUInt32(), 0, HIGHGUID_GUILD));                // guild guid
    else
        *data << uint64(0);
...
void Player::RegenerateHealth()
{
    ...
    {
        addvalue = 0.015f * ((float)GetMaxHealth()) * HealthIncreaseRate; //2 secs: 0.75*2
        if (!isInCombat())
        {
...
{
    sLog->outInfo(LOG_FILTER_PLAYER, "Sending Action Buttons for '%u' spec '%u'", GetGUIDLow(), GetActiveSpec());
    ...
    */
    ...
    GetSession()->SendPacket(&data);
    sLog->outInfo(LOG_FILTER_PLAYER, "Action Buttons for '%u' spec '%u' Sent", GetGUIDLow(), GetActiveSpec());
}
...
void Player::DuelComplete(DuelCompleteType type)
{
    ...
    data << uint8(type != DUEL_INTERRUPTED);
    GetSession()->SendPacket(&data);
    ...
    if (type != DUEL_INTERRUPTED)
    {
        ...
        data << uint8(type != DUEL_WON);                   // 0 = just won; 1 = fled
...
void Player::_ApplyItemBonuses(ItemTemplate const* proto, uint8 slot, bool apply, bool only_level_scale /*= false*/)
{
    ...
    for (uint8 i = 0; i < MAX_ITEM_PROTO_STATS; ++i)
    {
        ...
        switch (statType)
        {
            ...
            {
                UnitMods index = UNIT_MOD_ATTACK_POWER_POS;
                UnitMods ranged_index = UNIT_MOD_ATTACK_POWER_RANGED_POS;
                if (float(val) < 0.f)
                {
                    index = UNIT_MOD_ATTACK_POWER_NEG;
                    ranged_index = UNIT_MOD_ATTACK_POWER_RANGED_NEG;
                }
                HandleStatModifier(index, TOTAL_VALUE, float(val), apply);
                HandleStatModifier(ranged_index, TOTAL_VALUE, float(val) > 0.f ? float(val) : -float(val), apply);
                break;
            }
            case ITEM_MOD_RANGED_ATTACK_POWER:
            {
                UnitMods index = UNIT_MOD_ATTACK_POWER_RANGED_POS;
                if (float(val) < 0.f)
                    index = UNIT_MOD_ATTACK_POWER_RANGED_NEG;
                HandleStatModifier(index, TOTAL_VALUE, float(val) > 0.f ? float(val) : -float(val), apply);
                break;
            }
...
void Player::_ApplyWeaponDamage(uint8 slot, ItemTemplate const* proto, ScalingStatValuesEntry const* ssv, bool apply)
{
    ...
    if (ssv)
    {
        ...
        int32 extraDPS = ssv->GetDPSAndDamageMultiplier(proto->SubClass, proto->Flags2 & ITEM_FLAGS_EXTRA_CASTER_WEAPON, &damageMultiplier);
        if (extraDPS)
        {
...
//                            break;
                        ...
                        case ITEM_MOD_ATTACK_POWER:
                        {
                            ...
                            HandleStatModifier(index, TOTAL_VALUE, float(enchant_amount), apply);
                            HandleStatModifier(ranged_index, TOTAL_VALUE, float(enchant_amount) > 0.f ? float(enchant_amount) : -float(enchant_amount), apply);
                        ...
                        case ITEM_MOD_RANGED_ATTACK_POWER:
                        {
                            ...
                            if (float(enchant_amount) < 0.f)
                                index = UNIT_MOD_ATTACK_POWER_RANGED_NEG;
                            HandleStatModifier(index, TOTAL_VALUE, float(enchant_amount) > 0.f ? float(enchant_amount) : -float(enchant_amount), apply);
...
bool Player::CanRewardQuest(Quest const* quest, bool msg)
{
    ...
    if (quest->HasFlag(QUEST_TRINITY_FLAGS_DELIVER))
    {
        ...
        for (uint8 i = 0; i < QUEST_REQUIRED_CURRENCY_COUNT; ++i)
            if (quest->RequiredCurrencyCount[i] && GetCurrency(quest->RequiredCurrencyId[i]) < quest->RequiredCurrencyCount[i])
                return false;
    }
...
void Player::_LoadCurrency(PreparedQueryResult result)
{
    if (result)
    {
        do
        {
            ...
            uint32 currencyId = fields[0].GetUInt16();

            const CurrencyTypesEntry* entry = sCurrencyTypesStore.LookupEntry(currencyId);
            if (!entry)
            {
                sLog->outError(LOG_FILTER_PLAYER_LOADING, "Player::_LoadCurrency: %s (GUID: " UI64FMTD " Low: %u) has not invalid currency id %u.", GetName(), GetGUID(), GetGUIDLow(), currencyId);
                continue;
            }
            ...
            cur.totalCount = fields[1].GetUInt32();;
            cur.weekCount = fields[2].GetUInt32();;

            m_currencies[currencyId] = cur;
        }
        while(result->NextRow());
    }
}
...
void Player::_SaveCurrency(SQLTransaction& trans)
{
    ...
    {
        if (itr->second.state == PLAYERCURRENCY_UNCHANGED)
            continue;
        ...
        if (itr->second.state == PLAYERCURRENCY_CHANGED)
        {
            ...
            stmt->setUInt32(0, itr->second.totalCount);
            stmt->setUInt32(1, itr->second.weekCount);
            stmt->setUInt32(2, GetGUIDLow());
            stmt->setUInt32(3, itr->first);
        }
        else if (itr->second.state == PLAYERCURRENCY_NEW)
        {
            stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_PLAYER_CURRENCY);
            stmt->setUInt32(0, GetGUIDLow());
            stmt->setUInt32(1, itr->first);
            stmt->setUInt32(2, itr->second.totalCount);
            stmt->setUInt32(3, itr->second.weekCount);
        }
...
void Player::SetInGuild(uint32 GuildId)
{
    if (GuildId)
    {
        _guildGUID = MAKE_NEW_GUID(GuildId, 0, HIGHGUID_GUILD);
        SetUInt64Value(OBJECT_FIELD_DATA, _guildGUID);
