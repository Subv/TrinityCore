...
enum GroupUpdateFlags
{
    ...
    GROUP_UPDATE_FLAG_AURAS             = 0x00000200,       // uint64 mask, for each bit set uint32 spellid + uint8 auraflags
    ...
    GROUP_UPDATE_FLAG_PET_AURAS         = 0x00040000,       // uint64 mask, for each bit set uint32 spellid + uint8 auraflags, pet auras...
    GROUP_UPDATE_FLAG_VEHICLE_SEAT      = 0x00080000,       // uint32 vehicle_seat_id (index from VehicleSeat.dbc)
    GROUP_UPDATE_FLAG_PHASE             = 0x00100000,       // uint32, uint32 phase, string (unconfirmed)
    GROUP_UPDATE_PET                    = 0x0007FC00,       // all pet flags
    GROUP_UPDATE_FULL                   = 0x000FFFFF,       // all known flags, GROUP_UPDATE_FLAG_PHASE not included here
};
